...
- при переходе с Нового плеера на Старый озвучка не проигрывается до перезагрузки страницы (не добавлена логика в чекбокс)
-->

<!-- теперь, добавляется небольшое эхо :cry: -->
<!-- если видео на паузе, то все равно начинается перевод (coursehunter), вероятно, связано с тем, что вернул audioPlayer.play для решения #840 -->

- Добавлена поддержка Coursetrain (coursetrain.net)
...
- Исправлена логика работы расширенного увеличения громкости с новым плеером
  <!-- Опция "Восстановление мультимедийных клавиш" переименована в "Использовать новый плеер" (ПЕРЕИМЕНОВАТЬ этот пункт, И ПЕРЕИМЕНОВАТЬ "Обходить Media CSP" в "Использовать только для обхода Media CSP") -->
